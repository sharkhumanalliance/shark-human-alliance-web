...
\- public registry/verification surfaces should not expose private access tokens; private certificate links use `/certificate/view?token=...`

\- public registry/verification surfaces use the human-facing `registry_code`; raw member UUIDs stay internal and old `/verify?id=<member-id>` links remain as a backward-compatible fallback.
...
\- Dynamic wanted OG images are generated by `app/og/wanted/route.tsx`; keep deterministic output for identical query params.
