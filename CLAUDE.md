...
- Pools (in `wanted.*` of messages): `toneCharges.<tone>` (8 per tone), `commonCharges` (10), `posterSubtitles.<tone>` (7 per tone), `caseDetails` (10 strings). Charges = seeded shuffle of tone+common pool, 3 picked; subtitle/case detail rotate via `hash % length`.
- Poster QR links to the short `/w?t=<tone>&n=<name>`, which redirects to the localized `/wanted/case` (compact URL keeps QR module density low). Internal CTAs link to `/purchase?tier=protected&gift=true&from=<canonical_source>&name=<encoded>` (`wanted_gift_cta`, `wanted_case_cta`, `wanted_footer_cta`); anonymous cases omit `name`/`by` so the "Unidentified Human" placeholder never prefills a purchase. `/purchase` (in `purchase-flow.tsx`) reads `name`, `gift` and `from` on mount; `from` is normalized and persisted to `sessionStorage["sha_attribution_source"]` so the `purchase` event on `/purchase/success` can re-emit it after Stripe's redirect. Legacy `ref=wanted` was removed — it was being silently rejected by the SHA-XXXX referral-code validator.
